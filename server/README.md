# Описание приложения ATSAP Cluster Server

## Общее описание

ATSAP Cluster Server - это серверное приложение, реализующее API для работы с объектами и юнитами через прокси-сервис Rightech. Приложение предоставляет RESTful API для получения информации об объектах и устройствах, а также для выполнения различных операций с ними.

Приложение построено на основе паттерна Dependency Injection с использованием библиотеки InversifyJS.

##### Основные компоненты:

- Контроллеры - обрабатывают HTTP запросы
- Сервисы - реализуют бизнес-логику
- DTO - объекты передачи данных
- Entities - бизнес-сущности

##### Технические детали:

- Язык программирования: TypeScript
- Фреймворк: Express.js
- Порт по умолчанию: 3001
- Используется Dependency Injection (InversifyJS)
- Валидация данных (class-validator)
- Логирование
- Обработка ошибок

---

## API

### Внешние API

1. **Rightech API** - используется для получения данных об объектах и моделях
   - Базовый URL: `https://dev.rightech.io/api/v1`
   - Требуется авторизация через Bearer токен (RIGHTECH_API_TOKEN)
   - Документация: [https://dev.rightech.io](https://dev.rightech.io)

### Внутренний API приложения

Приложение предоставляет следующие эндпоинты:

##### Units (Юниты)
- `GET /api/v1/units/list` - Получение списка юнитов

##### Objects (Объекты)
- `POST /api/v1/objects/list` - Получение списка объектов

## 📝 Контракты для подключения

#### 1. Получение списка устройств
- **Метод**: GET
- **URL**: `/api/v1/units/list`
- **Ответ**: 
  ```json
  {
    "unitsList": [
      {
        "id": "string",
        "name": "string",
        "description": "string"
      }
    ]
  }
  ```

#### 2. Получение списка объектов
- **Метод**: POST
- **URL**: `/api/v1/objects/list`
- **Тело запроса**:
  ```json
  {
    "modelId": "string",
    "objectId": "string"
  }
  ```
- **Ответ**:
  ```json
  {
    "objectsList": [
      {
        "id": "string",
        "name": "string",
        "description": "string",
        "value": "any",
        "topic": "string",
        "type": "SENSOR | AUTOMATION",
        "sensorType": "HUMIDITY | TEMPERATURE",
        "sensorValueSymbol": "string"
      }
    ]
  }
  ```

---

## 🚀 Как использовать?

1. Клонируйте репозиторий:
    ```
    https://github.com/efnushtaev/atsap-mqtt-brocker-proxy.git
    cd atsap-mqtt-brocker-proxy
    ```
2. Установите зависимости:
    ```
    yarn install
    ```
3. Создайте файл .env в корне проекта и добавьте необходимые переменные окружения (см. Переменные окружения).
4. Соберите проект:
    ```
    yarn run build
    ```
5. Запустите приложение:
    ```
    yarn start
    ```

После запуска приложение будет доступно по адресу http://localhost:8001

### Запуск через Docker

1. Соберите Docker-образ:
    ```
    docker build -t atsap-mqtt-brocker-proxy .
    ```
2. Запустите контейнер:
    ```
    docker run --env-file .env -p 3000:8001 atsap-mqtt-brocker-proxy
    ```
Приложение будет доступно по адресу http://localhost:3000.

---

## Переменные окружения

Пример файла `.env`:


    RIGHTECH_API_TOKEN=your_rightech_api_token    // Токен для доступа к Rightech API.
    NODE_ENV=production                           // Среда выполнения (development, production).
    PORT=8001                                     // Порт, на котором запускается приложение.

    
